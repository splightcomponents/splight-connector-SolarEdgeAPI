...
class Main(AbstractComponent):
    ...
    energy_checkpoint = pytz.timezone("utc").localize(
    ...
    def task(self) -> None:
        ...
        for site_reader in self._mappings["SiteReader"]:
            if site_reader.resource == "power":
                ...
                start_time = (datetime.now() - timedelta(days=self._DEFAULT_CHECKPOINT_BACKWARDS_DAYS)
                              )
                resource = site_reader.resource

                data = self.client.get_power(site_id=site_reader.site_id,
                ...
                if data:
                    ...
                    )for value in data[site_reader.resource]["values"] if value["value"] is not None]
            ...
            if site_reader.resource == "energy":
                ...
                start_date = (datetime.now() - timedelta(days=self._DEFAULT_CHECKPOINT_BACKWARDS_DAYS)
                              )
                resource = site_reader.resource

                data = self.client.get_energy(site_id=site_reader.site_id, start_date=start_date, end_date=end_date
                                              )
                data_to_save = []
                if data:
                    data_to_save = [Number(
                        ...
                        value=value["value"],
                        timestamp=value["date"],
                    )for value in data[site_reader.resource]["values"] if (value["value"] is not None and value["date"] > self.energy_checkpoint)]
                    self.datalake_client.save(instances=data_to_save)
                    if data_to_save:
                        self.energy_checkpoint = max(
                            self.energy_checkpoint, data_to_save[-1].timestamp.strftime(
                                '%Y-%m-%d %H:%M:%S'))
        ...
        for inverter_id in inverter_readers_grouped_by_id:
            ...
            startTime = (datetime.now() - timedelta(days=self._DEFAULT_CHECKPOINT_BACKWARDS_DAYS)
                         ).strftime('%Y-%m-%d %H:%M:%S')
            ...
            for inverter_reader in inverter_readers_grouped_by_id[inverter_id]:
                ...
                if "." in inverter_reader.resource:
                    ...
                    data_to_save = [Number(asset=inverter_reader.asset.id,            attribute=inverter_reader.attribute.id,            value=value[phase][data_key],
                                           timestamp=value["date"],
                                           ) for value in data["data"]["telemetries"] if phase in value and value[phase].get(data_key) is not None]
                    logger.info(
                        f"resource:{inverter_reader.resource}------data length{len(data_to_save)}---------------------------")
                else:
                    # inverter data
                    data_to_save = [Number(asset=inverter_reader.asset.id,            attribute=inverter_reader.attribute.id,            value=value[inverter_reader.resource],
                                           timestamp=value["date"],
                                           ) for value in data["data"]["telemetries"] if value.get(inverter_reader.resource) is not None]
                    self.datalake_client.save(instances=data_to_save)
                    logger.info(
                        f"resource:{inverter_reader.resource}------data length{len(data_to_save)}---------------------------")

    def handle_mapping_create(self, reader: Union[SiteReader, InverterReader], mapping_type) -> None:
        try:
            self._mappings[mapping_type].append(
                reader)
            logger.info(
                f"mapping added {self.site_attribute_to_dict(reader)}")
    ...
    def handle_inverter_reader_delete(self, site_attribute_to_delete: InverterReader) -> None:
        if site_attribute_to_delete in self._mappings['SiteReader']:
    ...
    def _fetch_resource(self, resources, url, params):
        try:
            response = requests.get(url, params=params)
            data = json.loads(response.text)
            # try:
            data = [Number(
                ...
                value=value["value"],
                timestamp=value["date"],
            )for value in data["power"]["values"] if value["value"] is not None]
            # except Exception:
            self.datalake_client.save(instances=data)
        except Exception:
            print("it failed when appending number")
            logger.error(f"Error: {str(Exception)}")
            logger.exception(f"Value not present or mistype")
            return
